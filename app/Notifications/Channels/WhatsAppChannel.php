...
class WhatsAppChannel
{
    ...
    public function send(object $notifiable, Notification $notification): void
    {
        ...
        if (!$phoneNumber) {
            Log::warning('WhatsApp notification skipped: No phone number found', [
        ...
        if (!$user) {
            Log::warning('WhatsApp notification skipped: Could not determine user', [
                'notifiable_type' => get_class($notifiable),
                'notifiable_id' => $notifiable->id,
            ]);
            return;
        ...
        $result = $this->whatsAppService->sendWhatsApp($phoneNumber, $message);
        ...
        $logData = [
            ...
            'cost' => 1.0, // Base cost per WhatsApp message
        ];
        ...
        if ($result['status'] && $user->notificationPreferences) {
            $user->notificationPreferences->deductWhatsAppCredit();
        }
